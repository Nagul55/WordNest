...
const Folder = ({ color = '#5227FF', size = 1, items = [], className = '', onClick, open: controlledOpen }: FolderProps) => {
  const maxItems = 3;
  ...
  const handleClick = () => {
    ...
    if (onClick) {
      setTimeout(() => {
        onClick();
      }, 400); // 400ms delay to let opening animation finish
    }
  };
  ...
  // CSS variables typing for custom style mapping
  const folderStyle = {
    '--folder-color': color,
