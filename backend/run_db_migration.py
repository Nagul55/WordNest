...
for cluster in ["aws-0", "aws-1"]:
    for region in regions:
        ...
        for port in [6543, 5432]:
            ...
            print(f"Trying cluster={cluster}, region={region}, port={port}...")
            try:
                ...
                # Silently fail if not found or connection error
                if "tenant/user" not in str(e):
                    print(f"Debug connection failure on {db_host}:{port}: {e}")
...
if connected and conn:
    try:
        cursor = conn.cursor()
        schema_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "supabase", "schema.sql"))
